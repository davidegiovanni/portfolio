...
export default function LocaleConfigurator({ availableLocales, currentLocale }: LocaleConfiguratorProps) {
  const { t } = useTranslationsContext()
  const location = useLocation()
  const [menuBarOpen, toggleMenuBar] = useState(false)

  const mainLocale = currentLocale.split("-")[0]
  const formattedLocale = (locale: string): string => {
    return `${locale.split("-")[0]} (${locale.split("-")[1]})`
  }
        ...
        </Menubar.Trigger>
        {
          menuBarOpen && (
            <Menubar.Content
              ...
              align={"end"}
              alignOffset={0}>
              <Menubar.Label className='LocaleConfigurator--label OverrideLocaleConfigurator--label'>
                {t("change_language")}
              </Menubar.Label>
